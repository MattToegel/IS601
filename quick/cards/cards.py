...
from cards.forms import CardFetchForm, CardForm, CardSearchForm
...
def list():
    ...
    query = """
    SELECT id, name, text, flavor_text, mana_cost, rarity, card_class, card_type, card_set,spell_school FROM IS601_Quick_Cards WHERE 1=1
    """
    args = {}
    where = ""
    if form.name.data:
    ...
    if result.status and result.rows:
        rows = result.rows
    return render_template("card_list.html", rows=rows, form=form)