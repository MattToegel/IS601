...
from .models import SimpleHistory, db
from .mycalc import AdvMyCalc

c = AdvMyCalc()  # here the state is persisted between requests (note it may be shared across users)
...
def do_calc():
    ...
    # TODO fetch histories and display last 10
    data = request.form
    iSTR = data.get("eq")
    if iSTR is not None:
        checks = AdvMyCalc.ops
        for check in checks:
            if check in iSTR:
                ...
                try:
                    ...
                    r = "ERROR"
                if current_user.is_authenticated and r != "ERROR":
                    # record the simple history of equation and result
                    sh = SimpleHistory(eq=iSTR, result=r)
                    db.session.add(sh)
                    try:
                        db.session.commit()
